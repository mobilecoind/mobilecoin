...
uint256 hashGenesisBlock("0x56b08587c0d59784f91aea05c16b9f5b4a235c2e753f664e7b37a61385a2eb42");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // mobilecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "mobilecoin Signed Message:\n";
...
{
    // mobilecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 168000); // mobilecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

//static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // mobilecoin: 3.5 days king
static const int64 nTargetTimespan = 10 * 60; // mobilecoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60;//1 * 60; // mobilecoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // mobilecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // mobilecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // mobilecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x763566bcfe6673ccd7e0fe93759d34aa2dd7dccbfa2b0a7380d397e3e1c51e75");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block steking
        const char* pszTimestamp = "NY Times 05/Oct/2014 Steve Jobs, Apple’s Visionary, Dies at 56";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1394889010;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085384114;


        if (fTestNet)
        {
            block.nTime    = 1392294020;
            block.nNonce   = 2086379048;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x8763648c3502ff4ebd5de7527775f41b71e8a590674f9293b2cdfcbf53c27ea6"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // mobilecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("mobilecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("mobilecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static mobilecoinMiner(CWallet *pwallet)
{
    printf("mobilecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mobilecoin-miner");
    ...
    try { loop {
        ...
        printf("Running mobilecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("mobilecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&mobilecoinMiner, pwallet));
}
